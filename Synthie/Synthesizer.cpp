	...
	for (list<CInstrument *>::iterator node = m_instruments.begin(); node != m_instruments.end();)
	{
		...
		if (instrument->Generate())
		{
			...
			for (int i = 0; i < 3; i++)
			{
				if (sends[i])
				{
					instrument->SetSend(i+1, 1 + i * 0.5);
			...
			for (int j = 0; j < 5; j++)
			{
				for (int c = 0; c < GetNumChannels(); c++)
				{
					channelframes[j][c] += instrument->Frame(c) * instrument->Send(j);
				}
			}
		...
		double chorusframes[2];
		double flangeframes[2];
		double compressframes[2];
		for (int i = 0; i < 2; i++)
		{
			chorusframes[i] = flangeframes[i] = compressframes[i]  = 0;
		}
